...
def master(stich,prob,seg,feature):
    ...
    with redirect_stdout(f):
        print('#!/bin/bash')
        print('jid1=$(sbatch --parsable sticher.sbatch)')
        print('jid2=$(sbatch --dependency=afterok:$jid1 --parsable prob_mapper.sbatch)')
        print('jid3=$(sbatch --dependency=afterok:$jid2 --parsable segmenter.sbatch)')
        print('jid4=$(sbatch --dependency=afterok:$jid3 --parsable feature_extractor.sbatch)')
    f.close()
...
class feature_extractor(object):
   ...
   #directory = '/n/groups/lsp/cycif/example_data/'
    ...
    #save the sbatch job script
    def save_sbatch_file(self):
        f =  open('feature_extractor.sbatch', 'w')
        with redirect_stdout(f):
            self.print_sbatch_file()
        f.close()
